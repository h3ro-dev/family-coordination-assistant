...
describe("Email proxy flow (integration)", () => {
  ...
  test("parent request -> email outreach -> inbound email -> options SMS to parent", async () => {
    ...
    expect(email.sent[0].to).toBe(SITTER_EMAIL);
    expect(email.sent[0].replyTo).toBe(`assistant+${familyId}@example.com`);
